/* GDSF_compute: greedy dual frequency size with compute */
...
namespace eviction {
class GDSF_compute : public abstractRank {
 public:
  GDSF_compute() = default;
...
cache_t *GDSF_compute_init(const common_cache_params_t ccache_params,
  ...
  cache->cache_init = GDSF_compute_init;
  cache->cache_free = GDSF_compute_free;
  cache->get = GDSF_compute_get;
  cache->find = GDSF_compute_find;
 ...
 */
static void GDSF_compute_free(cache_t *cache) {
  delete reinterpret_cast<eviction::GDSF_compute *>(cache->eviction_params);
...
static cache_obj_t *GDSF_compute_insert(cache_t *cache, const request_t *req) {
  ...
  // if (!GDSF_compute_can_insert(cache, req)) return nullptr;
