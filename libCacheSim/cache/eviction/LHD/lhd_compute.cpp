...
LHDCompute::LHDCompute(int _associativity, int _admissions, cache_t* _cache)
...
candidate_t LHDCompute::rank(const request_t* req) {
  uint64_t victim = -1;
...
void LHDCompute::update(candidate_t id, const request_t* req) {
  ...
  tag->size = 1;
  tag->compute_intensity = req->obj_cost;
...
void LHDCompute::replaced(candidate_t id) {
  auto itr = indices.find(id);
...
void LHDCompute::updateClass(Class& cl) {
  cl.totalHits = 0;
  cl.totalEvictions = 0;
...
void LHDCompute::modelHitDensity() {
  for (uint32_t c = 0; c < classes.size(); c++) {
    rank_t totalEvents =
...
void LHDCompute::dumpClassRanks(Class& cl) {
  if (!DUMP_RANKS) {
    return;
  }
...
void LHDCompute::adaptAgeCoarsening() {
