...
typedef struct {
  void *LHD_compute_cache;

  int associativity;
  int admission;

  candidate_t to_evict_candidate;
} LHD_compute_params_t;
...
static void LHD_compute_free(cache_t *cache);
static bool LHD_compute_get(cache_t *cache, const request_t *req);

static cache_obj_t *LHD_compute_find(cache_t *cache, const request_t *req,
                                     const bool update_cache);
static cache_obj_t *LHD_compute_insert(cache_t *cache, const request_t *req);
static cache_obj_t *LHD_compute_to_evict(cache_t *cache, const request_t *req);
static void LHD_compute_evict(cache_t *cache, const request_t *req);
static bool LHD_compute_remove(cache_t *cache, const obj_id_t obj_id);
static int64_t LHD_compute_get_occupied_byte(const cache_t *cache);
static int64_t LHD_compute_get_n_obj(const cache_t *cache);
...
cache_t *LHD_compute_init(const common_cache_params_t ccache_params,
  ...
  if (ccache_params.default_ttl < 30 * 86400) {
    ERROR("LHD_compute does not support expiration\n");
    abort();
  }
#endif

  cache_t *cache =
      cache_struct_init("LHD_compute", ccache_params, cache_specific_params);
  cache->cache_init = LHD_compute_init;
  cache->cache_free = LHD_compute_free;
  cache->get = LHD_compute_get;
  cache->find = LHD_compute_find;
  cache->insert = LHD_compute_insert;
  cache->evict = LHD_compute_evict;
  cache->to_evict = LHD_compute_to_evict;
  cache->remove = LHD_compute_remove;
  cache->can_insert = cache_can_insert_default;
  cache->get_occupied_byte = LHD_compute_get_occupied_byte;
  cache->get_n_obj = LHD_compute_get_n_obj;
  ...
  auto *params = my_malloc(LHD_compute_params_t);
  memset(params, 0, sizeof(LHD_compute_params_t));
 ...
 */
static void LHD_compute_free(cache_t *cache) {
  auto *params = static_cast<LHD_compute_params_t *>(cache->eviction_params);
  auto *lhd = static_cast<repl::LHDCompute *>(params->LHD_compute_cache);
  delete lhd;
  free(cache->to_evict_candidate);
  my_free(sizeof(LHD_compute_params_t), params);
